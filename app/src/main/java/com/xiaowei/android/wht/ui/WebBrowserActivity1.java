...
public class WebBrowserActivity1 extends BaseNoTitleBarActivity
  ...
  String homeUrl = "file:///android_asset/t.html";//Config.indexUrl;//
  ...
  protected void onResume() {
    ...
    webview.loadUrl(
        "http://121.40.126.229:8082/wht/phone_releaseCaseIndex.action?userid=f9a8fe655846b399015846c603980002&caseclass=0");//file:///android_asset/test.html");
  }
  ...
  private void init() {
    ...
    webview.addJavascriptInterface(WebBrowserActivity1.this, "whtApp");// 在html页面中
    // html能访问的方法：
  ...
  public void nativeGoBack() {
    ...
    runOnUiThread(new Runnable() {
      ...
      public void run() {
        ...
        if (WebBrowserActivity1.this.webview.canGoBack()) {
          WebBrowserActivity1.this.webview.goBack(); // //回退到上一个页面
        } else {
          WebBrowserActivity1.this.finish();
        }
      }
  ...
  public void appRegMeet(final String meetingId) {
    ...
    runOnUiThread(new Runnable() {
      ...
      public void run() {
        UserInfo ui = ApplicationTool.getInstance().getUserInfo();
        if (ui == null) {
          Intent it = new Intent(WebBrowserActivity1.this, LoginActivity.class);
          it.putExtra("meetingId", meetingId);
          startActivityForResult(it, requestCodeLogin);
        } else {
          String params =
              "mobile=" + ui.getMobile() + "&userid=" + ui.getId() + "&meetid=" + meetingId;
  ...
  public void appGoPay(final String userId, final String orderId, final String mny) {
    ...
    runOnUiThread(new Runnable() {
      ...
      @Override
      public void run() {
        Intent it = new Intent(WebBrowserActivity1.this, PayActivity.class);
  ...
  public void doWeixinPay(final String payRequestStr) {
    payOk = false;
    dialog = ProgressDialog.show(WebBrowserActivity1.this, "提示信息", "正在调起微信支付...");
    //		HlpUtils.showToast(getApplicationContext(), payRequestStr);
    runOnUiThread(new Runnable() {

      @Override
      public void run() {
        final IWXAPI msgApi = WXAPIFactory.createWXAPI(WebBrowserActivity1.this, null);
        PayReq req = JSON.parseObject(payRequestStr.replace("\"package\"", "\"packageValue\""),
            PayReq.class);
