...
public class MeetingApply extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//右滑退出
		SildingFinishLayout mSildingFinishLayout = (SildingFinishLayout) findViewById(R.id.sildingFinishLayout_meeting_apply); 
	...
	private void initListeners() {
		...
		//选择科室
		findViewById(R.id.rl_meeting_apply_administrative_office).setOnClickListener(new OnClickListener() {

			@Override
			public void onClick(View v) {

				startActivityForResult(new Intent(MeetingApply.this, SectionOfficeChooseActivity.class), 513);
		...
		findViewById(R.id.rl_meeting_apply_area).setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				startActivityForResult(new Intent(MeetingApply.this, AreaChooseActivity.class), RESULTCODE_AREA_CHOOSE);
				overridePendingTransition(R.anim.in_right,0);
			}
		});
		...
		btnApply.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				String dept = etAdministrativeOffice.getText().toString().trim();
				if(dept.isEmpty()){
					mToast.showToast(MeetingApply.this, "请选择科室");
					return;
				}
				String doctorno = etExecutePermit.getText().toString().trim();
				...
				String duty = tvPostTitle.getText().toString().trim();
				if(dept.isEmpty()){
					mToast.showToast(MeetingApply.this, "请选择职务");
					return;
				}
				
				meetingApply(meetid, username, sex, hospital, dept, area.getAreaname(), area.getId(), doctorno
	...
	private void meetingApply(final String meetid, final String username, final int sex, final String hospital
				...
				try {
					...
					if (!HlpUtils.isEmpty(s)){
						...
						if(hr != null){
							if(hr.isSuccess()){
								if(!isDestroy){
									MeetingNoticeDoctor meeting = JSON.parseObject(hr.getData().toString(), MeetingNoticeDoctor.class);
									if(mny <= 0){
										//不需缴费，跳转成功页面
										...
										it.putExtra("userId", new SpData(MeetingApply.this).getStringValue(SpData.keyId, null));
								...
								runOnUiThread(new Runnable(){  
									@Override  
									public void run() {  
										Toast.makeText(MeetingApply.this, hr.getData().toString(), Toast.LENGTH_SHORT).show();
									}  
								});
							}
						}
				...
				try {
					...
					if (!HlpUtils.isEmpty(s)){
						...
						if(hr != null){
							if(hr.isSuccess()){
								if(!isDestroy){
									final DoctorPerson doctorPerson = JSON.parseObject(hr.getData().toString(), DoctorPerson.class);
									runOnUiThread(new Runnable(){  
										@Override  
										public void run() { 
