...
public class MyMessageActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//右滑退出
		SildingFinishLayout mSildingFinishLayout = (SildingFinishLayout) findViewById(R.id.sildingFinishLayout_my_message); 
	...
	private void initListeners() {
		//返回
		findViewById(R.id.iv_my_message_back).setOnClickListener(new OnClickListener() {
	...
	private void initViews() {
		listView = (SwipeListView) findViewById(R.id.listview_my_message);
	...
	private void reload(String text){
		if (loadingDialog == null){
			loadingDialog = Utils.createLoadingDialog(MyMessageActivity.this, text);
		}
			...
			public void run() {
				...
				try {
					String s = DataService.updateMyNotice(getApplicationContext()
							,new SpData(MyMessageActivity.this).getStringValue(SpData.keyPhoneUser, null)
					...
					if (!HlpUtils.isEmpty(s)){
						...
						if(hr != null){
							if(hr.isSuccess()){
								...
								runOnUiThread(new Runnable(){  
									@Override  
									public void run() {  
										Toast.makeText(MyMessageActivity.this, hr.getData().toString(), Toast.LENGTH_SHORT).show();
									}  
								});
							}
						}else{
						...
						runOnUiThread(new Runnable(){  
							@Override  
							public void run() {  
								Toast.makeText(MyMessageActivity.this, "请检查网络后重试！", Toast.LENGTH_SHORT).show();
							}  
						});
					}
				}catch (Exception he) {
	...
	class SwipeListViewListener extends BaseSwipeListViewListener{
		...
		public void onClickFrontView(int position) {
			...
			if (message.getOptye().equals("OPEN_DOCTOR_REFER")){
				intent = new Intent(MyMessageActivity.this, PatientStateActivity.class)
			...
			//患者，我的转诊
				...
				intent = new Intent(MyMessageActivity.this, PatientMyReferralInfoActivity.class)
				.putExtra("msgType", message.getMsgtype())
				.putExtra("id", message.getDataid())
				.putExtra("isComNotifi", true);
			}
			//患者，待接诊
				...
				intent = new Intent(MyMessageActivity.this, PatientMyReferralInfoDoneActivity.class)
				.putExtra("msgType", message.getMsgtype())
				.putExtra("id", message.getDataid())
				.putExtra("isComNotifi", true);
			}
