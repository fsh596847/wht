...
/**
 * Created by johnwatson on 4/21/16.
 */
public abstract class MyBaseQuickAdapter<T, H extends BaseAdapterHelper> extends BaseAdapter {
  protected static final String TAG = MyBaseQuickAdapter.class.getSimpleName();
   ...
   */
  public MyBaseQuickAdapter(Context context, int layoutResId) {
   ...
   */
  public MyBaseQuickAdapter(Context context, int layoutResId, List<T> data) {
  ...
  public void addAll(int postion, List<T> elem) {
    data.addAll(postion, elem);
    notifyDataSetChanged();
  }
   ...
   * display the correct data, this method can create a new view. Heterogeneous lists can specify
   * their number of view types, so that this View is always of the right type (see {@link
